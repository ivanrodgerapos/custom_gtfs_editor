...
class FrequencyService
{
    public static function add_frequency($request) {
        ...
        try {
            ...
            $audit_trail_data = AuditTrailHelper::format(1,'frequency','add', $data);
    ...
    public static function update_frequency_by_id ($request) {
        ...
        try {
            ...
            if ($record_exists) {
                ...
                $old_data =  Frequency::instance()->get_by_id($id)[0];

                $result = Frequency::instance()->update_frequency_by_id($data);

                unset($data['id']);
                $audit_trail_data = AuditTrailHelper::format(1,'frequency','update', $data, $old_data);
                $audit_trail->add($audit_trail_data);
